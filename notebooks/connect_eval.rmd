...
student_surveys <- na.omit(student_surveys)
for (i in 1:ncol(student_surveys)) {
  print(
    ggplot(student_surveys, aes(x = student_surveys[[i]], fill = student_surveys[[i]])) +
      ...
      scale_fill_manual(values = rep_len(
                   ...
                   nrow(student_surveys)
                 ))+
      ylab("Count")+
      ggtitle(student_survey_title[i])
  )
  filename = paste("images/student_surveys/", i, ".png", sep = "")
...
text <-
  pre_standarize$`What aspect(s) of the program/course were most helpful to you in achieving your learning goals?`
...
organization_survey_spring_2019 <-
  read_csv("data/Organization Survey_Spring 2019.csv")
organization_survey_spring_2019 <-
  mutate(organization_survey_spring_2019, cohort = "Spring 2019")
organization_survey_summer_2019 <-
  read_csv("data/Organization Survey_Summer 2019.csv")
organization_survey_summer_2019 <-
  mutate(organization_survey_summer_2019, cohort = "Summer 2019")
organization_survey_fall_2019 <-
  read_csv("data/Organization Survey_Fall 2019.csv")
organization_survey_fall_2019 <-
  mutate(organization_survey_fall_2019, cohort = "Fall 2019")
organization_survey_spring_2020 <-
  read_csv("data/Organization Survey_Spring 2020.csv")
organization_survey_spring_2020 <-
  mutate(organization_survey_spring_2019, cohort = "Spring 2020")
...
organization_surveys <- na.omit(organization_surveys)
# GGPLOTS
for (i in 1:ncol(organization_surveys)) {
  ...
  print(
    ggplot(
      organization_surveys,
      aes(x = organization_surveys[[i]], fill = organization_surveys[[i]])
    ) +
      geom_histogram(stat = "count") +
      theme(
        plot.title = element_text(
  ...
  filename = paste("images/organization_surveys/", i, ".png", sep = "")
  ggsave(filename, dpi = 300)
}
...
# Create a vector containing only the text
text <-
  prestandardize$`Please describe how you intend to use or build upon the work done by your consultant(s) (if at all).`
...
df$word <- as.character(df$word)
target <-
  c("also", "project", "really", "will", "work", "use", "program")
"%ni%" <- Negate("%in%")
df <- filter(df, word %ni% target)
df$word <- as.factor(df$word)
...
# Create a vector containing only the text
text <-
  prestandardize$`What suggestions for improvement do you have for future iterations of this program or course?`
...
# Create a vector containing only the text
text <-
  prestandardize$`Is there anything else that you would like to share about your experience? (Optional)`
...
ggsave("images/project_sectors.png", dpi = 500)
```
...
ggsave("images/project_type.png", dpi = 500)
```
...
print("Contributing Organizations")
print(mean(contributing_yes$survey_score), "Average Survey Score")
print(sd(contributing_yes$survey_score), "Standard Deviation")

contributing_no <- filter(organization_surveys, contributed == "no")
print("Non-contributing Organizations")
print(mean(contributing_no$survey_score), "Average Survey Score")
print(sd(contributing_no$survey_score), "Standard Deviation")
```
...
p <- ggplot(data = org_by_cohort, aes(x = cohort, y = average_score, fill = cohort)) +
  ...
  scale_fill_manual(values = c("#999999", "#E69F00", "#56B4E9", "#398814")) +
...
org_summary <- org_summary %>% mutate(cohort = fct_relevel(cohort, c("Spring 2019", "Summer 2019", "Fall 2019", "Spring 2020")))
...
student_plusorgs <- read_csv("data/student_plusorgs.csv")
student_plusorgs$cohort <- as.factor(student_plusorgs$cohort)
cohort <- select(student_plusorgs, cohort)
student_plusorgs <- student_plusorgs[,4:15]
student_plusorgs <- student_plusorgs[, -10]
student_plusorgs <- cbind(student_plusorgs, cohort)
question_title <- colnames(student_plusorgs)


# Reshape the data to get the mean and standard deviation from each question
library(plyr)
library(reshape2)
melted <- melt(student_plusorgs, id.vars = c("cohort"))
student_summary <- ddply(melted, c("cohort", "variable"), summarise,
  mean = mean(value), sd = sd(value),
  sem = sd(value) / sqrt(length(value))
)

student_summary <- student_summary %>% mutate(cohort = fct_relevel(cohort, c("Spring 2019", "Summer 2019", "Fall 2019", "Spring 2020")))
