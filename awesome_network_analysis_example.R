...
top_packages <- data %>%
  dplyr::filter(language %in% popular_languages) %>%
  dplyr::group_by(pkg_name) %>%
  dplyr::summarize(total_code = sum(code)) %>%
  dplyr::arrange(desc(total_code)) %>%
  head(number_of_pkgs) %>%
  dplyr::select(pkg_name, total_code)
...
top_languages_per_pkg <- data %>%
  ...
  dplyr::arrange(pkg_name, desc(code)) %>%
  dplyr::group_by(pkg_name) %>%
  dplyr:: mutate(
  ...
  dplyr::ungroup() %>%
  dplyr::select(language, pkg_name, code, total_code, main)
...
edges2 <- top_languages_per_pkg %>%
  dplyr::count(language, wt = code, name = 'total_code') %>%
  transmute(
    from = '',
    to = language,
    total_code,
